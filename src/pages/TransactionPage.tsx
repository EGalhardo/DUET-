        ...
        <div id="transaction-card" className="bg-white rounded-3xl border border-gray-200 overflow-hidden relative">
          {loading && (
            <div className="absolute inset-0 bg-white/80 z-40 flex flex-col items-center justify-center">
              ...
              <p className="text-[10px] font-black text-[#0c1e56] uppercase tracking-widest animate-pulse">A Processar Operação...</p>
            </div>
          )}
          ...
          <div id="transaction-header" className="px-5 py-4 border-b border-gray-200 bg-transparent flex items-center justify-between">
            ...
            <span id="secure-badge" className="flex items-center gap-1 text-[9px] text-green-600 font-black uppercase tracking-widest border border-green-200/50 bg-green-50 px-2 py-0.5 rounded-full">
          ...
          <form id="transaction-form" className="p-6 space-y-5" onSubmit={handleTransactionSubmit}>
            <div>
              <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 px-1">Valor (Kz)</label>
              ...
              {error && (
                <p className="mt-2 text-[10px] font-bold text-red-500 uppercase tracking-tight px-1">
                  {error}
                </p>
              )}
            </div>
            ...
            {type === 'depositar' && (
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 px-1">Método de Depósito</label>
                  <div className="relative">
                    <select 
                      id="method-select"
                      value={method}
                ...
                {method === 'ref_multicaixa' && (
                  <div className="p-4 bg-slate-50 border border-slate-200/70 rounded-2xl space-y-2.5 animate-pop-in">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-500 font-bold uppercase text-[9px]">Entidade</span>
                    ...
                    <div className="flex justify-between items-center text-xs border-t border-slate-200/50 pt-2.5">
                      <span className="text-slate-500 font-bold uppercase text-[9px]">Referência</span>
                      <span className="font-black text-slate-800 tracking-wider">992 001 023</span>
                    </div>
                    <p className="text-[9px] text-slate-400 leading-normal text-center pt-1.5 uppercase font-bold tracking-tight">
                ...
                {method === 'transferencia' && (
                  <div className="p-4 bg-slate-50 border border-slate-200/70 rounded-2xl space-y-2.5 animate-pop-in">
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-500 font-bold uppercase text-[9px]">Titular</span>
                    ...
                    <div className="flex flex-col text-xs border-t border-slate-200/50 pt-2.5">
                      ...
                      <span className="font-mono font-black text-slate-800 text-[10px] bg-slate-200/50 p-2 rounded-lg select-all">
            ...
            {type === 'levantar' && (
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 px-1">IBAN de Destino (AO06)</label>
                  <input 
                    type="text"
                    value={iban}
                    onChange={(e) => {
                    ...
                    required
                  />
                  <p className="text-[9px] text-slate-400 mt-1.5 px-1 uppercase font-bold tracking-tight">O IBAN deve conter exatamente 21 caracteres no total e iniciar com "AO06".</p>
                </div>
              </div>
            )}
            ...
            {type === 'transferir' && (
              <div className="space-y-4">
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 px-1">Destinatário (ID do Jogador ou Telemóvel)</label>
                  <input 
                    type="text"
                    value={destTarget}
                    onChange={(e) => {
                    ...
                    required
                  />
                  <p className="text-[9px] text-slate-400 mt-1.5 px-1 uppercase font-bold tracking-tight">A transferência será enviada instantaneamente para a carteira do utilizador de destino.</p>
                </div>
              </div>
            )}
            ...
            <div id="total-preview" className="bg-amber-50/50 rounded-2xl p-4 border border-amber-200/40">
               <div className="flex items-center justify-between">
                 <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Saldo a processar</span>
                 <span className="text-lg font-black text-[#0c1e56]">Kz {amount ? Number(amount).toLocaleString() : '0'}</span>
               </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button 
                ...
                onClick={() => navigate(-1)}
                disabled={loading}
                className="flex-1 py-2.5 rounded-[1.2rem] border-2 border-gray-200 font-black text-slate-600 hover:bg-gray-50 transition-all uppercase tracking-widest text-[10px] cursor-pointer"
              >
                Cancelar
              </button>
              <button 
                ...
                type="submit"
                disabled={loading}
                className="flex-1 py-2.5 rounded-[1.2rem] bg-[#FFB10A] text-white font-black hover:bg-[#FFC000] transition-all uppercase tracking-widest text-[10px] shadow-lg shadow-orange-50 cursor-pointer"
              >
                Confirmar
              </button>
            </div>
