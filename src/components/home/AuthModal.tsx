...
import { X, LogIn, Globe, UserPlus, Key, Mail, Lock, User, Check, ShieldAlert } from 'lucide-react';
...
export default function AuthModal({ isOpen, onClose, initialTab }: AuthModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'login') {
      ...
      login();
      setSuccessMsg('Sessão iniciada com sucesso como Edlasio Galhardo!');
      setTimeout(() => {
        onClose();
      }, 1500);
    } 
  ...
  const handleSocialLogin = (platform: string) => {
    login();
    setSuccessMsg(`Conectado com sucesso via ${platform}!`);
    setTimeout(() => {
      onClose();
    }, 1500);
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="p-6 md:p-8 space-y-6 bg-[#F8FAFC]">
              {successMsg ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  {errorMsg && (
                    <div className="p-3 bg-red-50 border border-red-200 text-red-600 text-[10px] font-bold rounded-xl flex items-center gap-2">
                  ...
                  {activeTab === 'signup' && (
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 pl-1">Nome Completo</label>
                  ...
                  {(activeTab === 'login' || activeTab === 'signup' || activeTab === 'reset') && (
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 pl-1">Endereço de E-mail</label>
                      <div className="relative">
                  ...
                  {(activeTab === 'login' || activeTab === 'signup') && (
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 pl-1">Senha</label>
                      <div className="relative">
                  ...
                  {activeTab === 'signup' && (
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 pl-1">Confirmar Senha</label>
                      <div className="relative">
                        <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                        <input
                          type="password"
                  ...
                  {activeTab === 'social' && (
                    <div className="space-y-3 pt-2">
                      <button
                        ...
                        className="w-full py-3.5 px-4 bg-white hover:bg-slate-50 border-2 border-slate-200 rounded-2xl font-black text-[10px] uppercase tracking-wider text-slate-700 transition-all flex items-center justify-center gap-2.5 active:scale-95 shadow-sm"
                      >
                      ...
                      <button
                        ...
                        className="w-full py-3.5 px-4 bg-[#1877F2] hover:bg-[#166FE5] rounded-2xl font-black text-[10px] uppercase tracking-wider text-white transition-all flex items-center justify-center gap-2.5 active:scale-95 shadow-sm"
                      >
                      ...
                      <button
                        ...
                        className="w-full py-3.5 px-4 bg-black hover:bg-neutral-900 rounded-2xl font-black text-[10px] uppercase tracking-wider text-white transition-all flex items-center justify-center gap-2.5 active:scale-95 shadow-sm"
                      >
                        <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  ...
                  {activeTab !== 'social' && (
                    <button
                      type="submit"
                      className="w-full py-4 mt-2 bg-[#FFB10A] text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-[#e69f09] active:scale-95 transition-all shadow-md text-center"
                    >
                    ...
                    {activeTab === 'login' && (
                      <>
                        <button
                          ...
                          className="text-[10px] text-slate-500 font-bold hover:text-[#091747] transition-colors"
                        >
            ...
            <div className="p-5 bg-white border-t border-gray-100 flex justify-end rounded-b-[2.5rem]">
              <button 
                onClick={onClose}
                className="w-full py-4 bg-transparent border-2 border-gray-300 text-slate-600 rounded-2xl font-extrabold text-[11px] uppercase tracking-widest hover:bg-slate-50 active:scale-95 transition-all text-center"
              >
                Cancelar
              </button>
            </div>
