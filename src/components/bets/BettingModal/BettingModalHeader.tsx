...
const BettingModalHeader: React.FC<BettingModalHeaderProps> = ({ match }) => {
  return (
    <div className="bg-[#091747] rounded-[2rem] p-6 text-white mb-6 relative overflow-hidden shadow-xl">
      <div className="absolute right-0 top-0 w-32 h-32 bg-[#FFB10A]/10 rounded-full blur-3xl" />
      
      <div className="flex flex-col items-center gap-4 relative z-10">
        <div className="flex items-center justify-center gap-8 w-full">
          <div className="flex flex-col items-center flex-1">
            <div className="w-14 h-14 md:w-20 md:h-20 bg-white rounded-2xl p-2 flex items-center justify-center shadow-lg border-2 border-white/10 mb-2">
              <img src={match.teamA.logo} alt={match.teamA.name} className="w-full h-full object-contain" />
            </div>
            <span className="text-[10px] font-black uppercase tracking-widest text-center h-8 flex items-center">
              {match.teamA.name}
            </span>
          </div>

          <div className="flex flex-col items-center">
            <div className="bg-white/10 px-4 py-1.5 rounded-full mb-2">
              <span className="text-[#FFB10A] text-xl font-black italic">VS</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 w-full mt-2">
          <div className="bg-white/5 rounded-xl py-2 px-3 flex items-center gap-2 border border-white/5">
